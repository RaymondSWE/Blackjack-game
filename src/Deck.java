    ...
    public void loadImages() {
        try {
            for (Card card : mCards)
                card.setImage(ImageIO.read(new File(card.getFullImagePath())).getScaledInstance(50, 73, BufferedImage.SCALE_SMOOTH));
            backCover = ImageIO.read(new File("Images\\Cardimages\\BACK COVER.png")).getScaledInstance(50, 73, BufferedImage.SCALE_SMOOTH);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
